-- Corrigir a descrição na versão correta do pay_sale (a que está sendo usada)
-- Esta é a versão com 6 parâmetros que está sendo chamada pelo sistema

CREATE OR REPLACE FUNCTION pay_sale(
    p_sale_id UUID,
    p_payments JSONB,
    p_stock_items JSONB DEFAULT '[]'::JSONB,
    p_credit_debit JSONB DEFAULT NULL::JSONB,
    p_change_amount NUMERIC DEFAULT 0,
    p_is_physical_mode BOOLEAN DEFAULT false
) RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
    v_payment JSONB;
    v_item JSONB;
    v_sale RECORD;
    v_tenant_id UUID;
    v_method TEXT;
    v_amount NUMERIC;
    v_bank_account_id UUID;
    v_account_active BOOLEAN;
    v_occurred_at TIMESTAMPTZ;
    v_customer_name TEXT;
    v_sale_items TEXT;
    v_description TEXT;
BEGIN
    -- Usar NOW() que já retorna em UTC
    v_occurred_at := NOW();

    -- Buscar venda
    SELECT * INTO v_sale FROM sales WHERE id = p_sale_id;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Sale not found: %', p_sale_id;
    END IF;
    
    -- Check if already paid
    IF v_sale.status = 'paid' THEN
        RAISE EXCEPTION 'Sale is already paid.';
    END IF;

    v_tenant_id := v_sale.tenant_id;

    -- Buscar nome do cliente se houver
    IF v_sale.customer_id IS NOT NULL THEN
        SELECT name INTO v_customer_name FROM clients WHERE id = v_sale.customer_id;
    END IF;

    -- Buscar itens da venda para a descrição
    SELECT string_agg(
        COALESCE(srv.name, prd.name), 
        ', '
    )
    INTO v_sale_items
    FROM sale_items si
    LEFT JOIN services srv ON srv.id = si.service_id
    LEFT JOIN products prd ON prd.id = si.product_id
    WHERE si.sale_id = p_sale_id;

    -- Build description
    IF v_customer_name IS NOT NULL AND v_sale_items IS NOT NULL THEN
        v_description := 'Venda - ' || v_customer_name || ' (' || 
            CASE 
                WHEN length(v_sale_items) > 50 THEN substring(v_sale_items, 1, 47) || '...'
                ELSE v_sale_items
            END || ')';
    ELSIF v_customer_name IS NOT NULL THEN
        v_description := 'Venda - ' || v_customer_name;
    ELSE
        v_description := 'Venda #' || substring(p_sale_id::text, 1, 8);
    END IF;

    -- Registrar pagamentos
    FOR v_payment IN SELECT * FROM jsonb_array_elements(p_payments) LOOP
        v_method := (v_payment->>'method')::TEXT;
        v_amount := (v_payment->>'amount')::NUMERIC;
        v_bank_account_id := (v_payment->>'bankAccountId')::UUID;

        -- Validate bank account exists and is active (skip for wallet methods)
        IF v_bank_account_id IS NULL THEN
            IF v_method NOT IN ('credit', 'fiado') THEN
                RAISE EXCEPTION 'Bank account ID is required for payment method: %', v_method;
            END IF;
        ELSE
            SELECT is_active INTO v_account_active
            FROM bank_accounts
            WHERE id = v_bank_account_id AND tenant_id = v_tenant_id;

            IF NOT FOUND THEN
                RAISE EXCEPTION 'Bank account not found: %', v_bank_account_id;
            END IF;

            IF NOT v_account_active THEN
                RAISE EXCEPTION 'Bank account is inactive: %', v_bank_account_id;
            END IF;
        END IF;

        -- Insert into sale_payments
        INSERT INTO sale_payments (
            sale_id, 
            method, 
            amount, 
            paid_at, 
            change_amount
        )
        VALUES (
            p_sale_id,
            v_method,
            v_amount,
            v_occurred_at,
            CASE WHEN LOWER(v_method) = 'cash' THEN p_change_amount ELSE 0 END
        );

        -- Insert into cash_movements with bank_account_id (only for monetary methods with account)
        IF LOWER(v_method) IN ('cash', 'pix', 'card', 'transfer', 'wallet') AND v_bank_account_id IS NOT NULL THEN
            INSERT INTO cash_movements (
                tenant_id,
                bank_account_id,
                type,
                amount,
                method,
                source_type,
                source_id,
                description,
                occurred_at,
                created_by
            )
            VALUES (
                v_tenant_id,
                v_bank_account_id,
                'IN',
                CASE WHEN LOWER(v_method) = 'cash' THEN v_amount - p_change_amount ELSE v_amount END,
                UPPER(v_method),
                'SALE',
                p_sale_id,
                v_description,  -- Usando a descrição melhorada
                v_occurred_at,
                auth.uid()
            );
        END IF;
    END LOOP;

    -- Atualizar status da venda
    UPDATE sales
    SET status = 'paid', updated_at = now()
    WHERE id = p_sale_id;

    -- Movimentar estoque (saída)
    IF p_stock_items IS NOT NULL AND jsonb_array_length(p_stock_items) > 0 THEN
        FOR v_item IN SELECT * FROM jsonb_array_elements(p_stock_items) LOOP
            INSERT INTO stock_movements (tenant_id, product_id, type, qty, reason, reference_type, reference_id, created_by)
            VALUES (
                v_tenant_id,
                (v_item->>'productId')::UUID,
                'out',
                (v_item->>'qty')::INTEGER,
                'sale',
                'sale',
                p_sale_id,
                'system'
            );

            UPDATE products
            SET current_stock = current_stock - (v_item->>'qty')::INTEGER,
                last_movement = now(),
                updated_at = now()
            WHERE id = (v_item->>'productId')::UUID;
        END LOOP;
    END IF;

    -- Atualizar appointment se houver
    IF v_sale.appointment_id IS NOT NULL THEN
        UPDATE appointments
        SET status = 'DONE',
            finalized_at = now(),
            updated_at = now()
        WHERE id = v_sale.appointment_id;
    END IF;

    -- Debitar crédito (Carteira) com itens da venda
    IF p_credit_debit IS NOT NULL AND (p_credit_debit->>'amount')::NUMERIC > 0 THEN
        INSERT INTO credit_movements (tenant_id, client_id, type, amount, origin, note)
        VALUES (
            v_tenant_id,
            (p_credit_debit->>'clientId')::UUID,
            'DEBIT',
            (p_credit_debit->>'amount')::NUMERIC,
            'WALLET',
            v_description  -- Usando a descrição melhorada também aqui
        );

        UPDATE clients
        SET credit_balance = credit_balance - (p_credit_debit->>'amount')::NUMERIC,
            updated_at = now()
        WHERE id = (p_credit_debit->>'clientId')::UUID;
    END IF;
END;
$$;
